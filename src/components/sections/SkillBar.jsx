import { useRef, useState, useEffect } from "react";

const SkillBar = ({ name, icon, iconColor, percent, isCustomIcon }) => {
  const fillRef = useRef(null);
  const [animated, setAnimated] = useState(false);

  useEffect(() => {
    const el = fillRef.current;
    if (!el) return;

    const observer = new IntersectionObserver(
      ([entry]) => {
        if (entry.isIntersecting && !animated) {
          el.style.width = `${percent}%`;
          setAnimated(true);
          observer.disconnect();
        }
      },
      { threshold: 0.3 }
    );

    observer.observe(el);
    return () => observer.disconnect();
  }, [percent, animated]);

  return (
    <div className="mb-7">

      {/* ── Label Row ── */}
      <div className="flex justify-between items-center mb-2">

        <span className="flex items-center gap-3 font-semibold text-text-heading text-[clamp(0.9rem,1.5vw,1.1rem)]">

          {/* Icon */}
          {isCustomIcon ? (
            <svg
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 640 640"
              className="w-7"
              style={{ fill: iconColor }}
            >
              <path d="M320 128C234.7 128 181.3 170.7 160 256C192 213.3 229.3 197.3 272 208C296.3 214.1 313.7 231.8 333 251.3C364.4 283.1 400.7 320 480 320C565.3 320 618.7 277.3 640 192C608 234.7 570.7 250.7 528 240C503.7 233.9 486.3 216.2 467 196.7C435.6 164.9 399.3 128 320 128zM160 320C74.7 320 21.3 362.7 0 448C32 405.3 69.3 389.3 112 400C136.3 406.1 153.7 423.8 173 443.3C204.4 475.1 240.7 512 320 512C405.3 512 458.7 469.3 480 384C448 426.7 410.7 442.7 368 432C343.7 425.9 326.3 408.2 307 388.7C275.6 356.9 239.3 320 160 320z" />
            </svg>
          ) : (
            <i
              className={`${icon} text-[1.8rem]`}
              style={{ color: iconColor, width: "28px" }}
            />
          )}

          {name}
        </span>

        <span className="font-semibold text-primary text-[0.95rem]">
          {percent}%
        </span>

      </div>

      {/* ── Bar ── */}
      <div className="w-full h-2.25 bg-border rounded-full overflow-hidden">

        <div
          ref={fillRef}
          className="h-full rounded-full bg-linear-to-br from-primary to-accent transition-[width] duration-1200 ease-in-out will-change-[width]"
          style={{ width: "0%" }}
        />

      </div>

    </div>
  );
};

export default SkillBar;